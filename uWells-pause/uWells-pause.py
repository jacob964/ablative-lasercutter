...
def gcode_move(x_move, y_move):
	f.writelines("G0 X" + str(x_move) + " Y" + str(y_move) + " F" + str(feedRate) + "\n")
	f.writelines("G4 P" + str(pauseTime) + "\n")

def gcode_ablate(dwellTime, laserPower, iterations):
	for i in range(0, iterations):
		f.writelines("M3 S" + str(laserPower) + " L" + str(dwellTime * 1000) + "\n")
		f.writelines("G4 P" + str(dwellTime + 10 + offTime) + "\n")
	f.writelines("M3 S0\n")
...
def gcode_rectangle(dwellTime, laserPower, iterations):
	...
	while y_total < rectWidth:
		while x_total < rectLength:
			gcode_ablate(dwellTime, laserPower, iterations)
			gcode_move(-1*x_dist, 0)
			x_total += x_dist
		
		x_overhang = (1-flag)/2 * x_dist/2
		gcode_move(x_total - x_overhang, -1*y_dist)
